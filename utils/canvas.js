...
function createHistoryChart(historyArray) {
    try {
        ...
        for (let i = 3; i <= 18; i++) {
            ...
            ctx.beginPath(); ctx.moveTo(chartX, y); ctx.lineTo(chartX + chartWidth, y); ctx.stroke();
        }
        for (let i = 0; i <= 20; i++) {
            const x = chartX + (i / 20) * chartWidth;
            ctx.beginPath(); ctx.moveTo(x, chart1Y); ctx.lineTo(x, chart1Y + chart1Height); ctx.stroke();
        }
        ...
        last20.forEach((h, i) => {
            ...
            const y = chart1Y + chart1Height - ((h.total - 3) / 15) * chart1Height;
        ...
        last20.forEach((h, i) => {
            ...
            ctx.fillText(h.total.toString(), x, y);
        });
        ...
        for (let i = 0; i <= 20; i++) {
            const x = chartX + (i / 20) * chartWidth;
            ctx.beginPath(); ctx.moveTo(x, chart2Y); ctx.lineTo(x, chart2Y + chart2Height); ctx.stroke();
        }
        ...
        const lines = [
            { name: 'Xí ngầu 1', color: '#8b5cf6', data: [] },
            { name: 'Xí ngầu 2', color: '#06b6d4', data: [] },
            { name: 'Xí ngầu 3', color: '#ec4899', data: [] }
        ];
        last20.forEach(h => {
        ...
        lines.forEach((line, i) => {
            ...
            ctx.beginPath(); ctx.arc(legendX, legendY, 6, 0, Math.PI * 2); ctx.fill();
            ctx.fillStyle = '#ffffff'; ctx.font = '12px Arial';
            ctx.textAlign = 'left'; ctx.fillText(line.name, legendX + 12, legendY + 4);
        });
        ctx.fillStyle = '#444444'; ctx.font = '10px Arial';
        ctx.textAlign = 'right'; ctx.fillText('Powered by mxtbot.com', 690, 490);
...
// client truyen vao de lay avatar bot
async function createProfileCard(user, userData, avatarUrl, client) {
    try {
        ...
        // ===== BOT LOGO: avatar tron + ten "emzy" goc tren trai =====
        try {
            if (client && client.user) {
                const botAvatarUrl = client.user.displayAvatarURL({ extension: 'png', size: 64 });
                const botAvatar = await loadImage(botAvatarUrl);
...
function createBowlLift(dice1, dice2, dice3, liftPercent = 0) {
    try {
        ...
        if (liftPercent < 100) {
            ...
            ctx.beginPath(); ctx.arc(centerX + 5, bowlY + 5, 150, 0, Math.PI * 2); ctx.fill();
            ctx.fillStyle = '#8B5A3C';
            ctx.beginPath(); ctx.arc(centerX, bowlY, 150, 0, Math.PI * 2); ctx.fill();
            ctx.strokeStyle = '#A0694F'; ctx.lineWidth = 8; ctx.stroke();
            ctx.fillStyle = 'rgba(255,255,255,0.3)';
            ctx.beginPath(); ctx.arc(centerX - 40, bowlY - 30, 50, 0, Math.PI * 2); ctx.fill();
        }
        return canvas.toBuffer('image/png');
    } catch (error) {
...
function createRevealDice(dice) {
    try {
        ...
        ctx.fillStyle = '#FFFFFF';
        ctx.beginPath(); ctx.ellipse(300, 220, 200, 100, 0, 0, Math.PI * 2); ctx.fill();
        ...
        dice.forEach((num, index) => {
            ...
            if (num === 0) {
                ...
                ctx.beginPath(); ctx.arc(pos.x, pos.y, 35, 0, Math.PI * 2); ctx.fill();
            } else {
...
function drawRealisticDice(ctx, number, x, y, size = 70) {
    ...
    (dots[number] || []).forEach(([dx, dy]) => {
        ctx.beginPath(); ctx.arc(x + dx, y + dy, dotSize, 0, Math.PI * 2); ctx.fill();
    });
}
